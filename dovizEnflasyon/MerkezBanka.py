...
# EVDS API bilgileri
series = "TP.DK.USD.S.YTL"  # Dolar kuru serisi
...
if response.status_code == 200:
    ...
    dolar = pd.read_csv(csv_data)
    dolar.dropna(inplace=True)
    dolar.set_index("Tarih", inplace=True)
    dolar.rename(columns={series.replace(".", "_"): "Dolar_Kuru"}, inplace=True)
    ...
    model = SARIMAX(dolar["Dolar_Kuru"], order=(p, d, q), seasonal_order=(P, D, Q, s))
    ...
    plt.plot(dolar.index, dolar["Dolar_Kuru"], label="Gerçek Değerler")
    ...
    plt.title("Dolar Kuru Tahminleri (SARIMA)")
    plt.xlabel("Tarih")
    ...
    forecast_df.to_csv("sarima_dolar_tahmin_2024_2025.csv")
    print("Tahmin sonuçları 'sarima_dolar_tahmin_2024_2025.csv' dosyasına kaydedildi.")
else:
